    ...
    return (
        <div className="cardslider">
            <div className="cardslider__main" style={{backgroundColor: background}}>
                ...
                <img className="cardslider__main-img" src={activeImg.length > 0 ? activeImg : process.env.REACT_APP_BACK_URL + image} alt={title} />
            </div>
            {/*Картинки из массива*/}
            <div className="cardslider__down">
                <div className="cardslider__down-item">
                    <img onClick={getNewImg} className="cardslider__down-img" src={process.env.REACT_APP_BACK_URL + image} alt={title} />
                </div>

                {images_arr.map((item, i) => (
                    <div key={i} className="cardslider__down-item">
                        <img onClick={getNewImg} className="cardslider__down-img" src={process.env.REACT_APP_BACK_URL + item.trim()} alt=""/>
                    </div>
                ))}
